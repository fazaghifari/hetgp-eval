...
class HomGP():
    """Wrapper for homoscedastic GP so everything are in the same API style
    """
    ...
    def predict(self, X, return_std=None):
        ...
        if return_std is None:
            ...
            mean, std_ep = self.model.predict(X, return_std=True)
            st = np.ones(shape=std_ep.shape) 
            if return_std.lower()=="epistemic":
                # Epistemic std
                std = std_ep
            elif return_std.lower()=="aleatoric":
                # Aleatoric std
                std = np.sqrt(np.exp(self.model.kernel_.theta[-1])) * st
            elif return_std.lower()=="total":
                # Full std (epistemic + aleatoric)
                var_ep = std_ep**2
                var_al = np.exp(self.model.kernel_.theta[-1]) * st
                std=np.sqrt(var_ep+var_al)
            else:
                # Return aleatoric and epistemic separately
                std_al = np.sqrt(np.exp(self.model.kernel_.theta[-1])) * st
                std = [std_al, std_ep]
